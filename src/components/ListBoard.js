    ...
    <div className="list-container">
        {!error && <>
                                ...
                                {boardData && column.cards.map((card, index) => {
                                        return (
                                            <Draggable key={card.cardID} draggableId={card.cardID} index={index}>
                                                {(provided) => (
                                                    <li {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef} className={
                                                        card.cardDeadline && card.cardDeadline.toDate().getTime() <= new Date(new Date().setHours(0,0,0,0)).getTime() ? "taskCard is-expired" : "taskCard"
                                                    } onDoubleClick={() => {
            ...
            <div className="taskColumn" style={{paddingTop: "4.5em"}}>
                {!isColumnAddOpen && <Button variant="outlined" style={{marginBottom: "1.4em"}} onClick={() => setIsColumnAddOpen(true)}>+ New Column</Button>}
                {isColumnAddOpen && <Button variant="outlined" style={{marginBottom: "1.4em"}} onClick={() => {
                    setIsColumnAddOpen(false)
                    setNewColumnName("")
                }}>Cancel</Button>}
                {isColumnAddOpen &&
                    <div className="column">
                        <form onSubmit={handleAddColumn}>
                            ...
                            <Button variant="contained" type="submit" >add Column</Button>
                        </form>
                    </div>}
            </div>
            </>
