...
export default function TaskEdit({
  ...
  const [deadline, setDeadline] = useState(
    sourceCard.cardDeadline
      ? sourceCard.cardDeadline.toDate().toString().toLocaleString("en-US")
      : null
  );
  const [cardDescription, setCardDescription] = useState(
    sourceCard.cardDescription
  );
  ...
  function handleEditButton(e) {
    ...
    if (isEdit.state) {
      ...
      let newBoardDataColumn = newBoardData.columns.find(
        (col) => col.columnID === sourceColumn
      );
      // get card from column.cards
      let newBoardDataColumnCard = newBoardDataColumn.cards.find(
  ...
  function handleAdd(e) {
    ...
    if (
      labelName &&
      !cardLabels.filter((label) => label.labelName === labelName).length > 0
    ) {
      setCardLabels((prevLabels) => [
        ...prevLabels,
        { labelID: uuidv4(), labelName, labelColor, labelTextColor },
      ]);
    }
    setNewCardLabelName("");
  ...
  function handleDeleteLabel(labelID) {
    setCardLabels((cardLabels) =>
      cardLabels.filter((label) => label.labelID !== labelID)
    );
  }
